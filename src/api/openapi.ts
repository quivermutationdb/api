...
export const OPENAPI = {
  ...
  paths: {
    ...
    "/bulk": { get: { summary: "Index of the bulk corpus (the whole census as files)", operationId: "getBulkIndex",
      description: "For a full or near-full pull, prefer this over /export.ndjson: the census is pre-rendered as one gzipped NDJSON file per rank in R2, so there is no database work and egress is free. Rows are byte-for-byte what /export.ndjson serves. Returns file names, row counts, sizes and sha256 checksums.",
      responses: { 200: { description: "Manifest + usage" }, 503: { description: "Corpus not published yet" } } } },
    "/bulk/{file}": { get: { summary: "One bulk corpus file", operationId: "getBulkFile",
      description: "Gzipped NDJSON for a rank (qmd-n{k}.ndjson.gz), or manifest.json / rank_stats.json / nicknames.json. Range requests are supported, so a multi-gigabyte download can resume (curl -C - -O). Served as application/gzip, NOT Content-Encoding: gzip, so the bytes on disk match the manifest's sha256.",
      parameters: [pathParam("file", "qmd-n{rank}.ndjson.gz | manifest.json | rank_stats.json | nicknames.json")],
